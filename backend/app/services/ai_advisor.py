...
def get_ai_advice(ingredient: str, price: str, budget: int = 10000):
    ...
    prompt = (
    f"너는 자취 10년 차 베테랑 선배야. 후배한테 돈 아끼는 꿀팁을 전수해 주듯이 아주 친절하고 재치 있게 조언해줘. "
    f"요리 재료 '{ingredient}'의 최저가가 {price}원이야. "
    f"나의 오늘 총 예산은 {budget}원인데, 대학생 자취생 입장에서 이 가격이 적당한지, "
    f"전체 예산 대비 비중은 어떤지 고려해서 '더 싸게 살 방법'이나 '대체 재료'를 딱 '한 문장'으로 이모지를 섞어서 말해줘."
)
